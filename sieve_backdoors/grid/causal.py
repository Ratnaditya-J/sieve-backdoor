...
def _fire_rate_steered(model, prompts, payload, layer, direction, alpha) -> float:
    hits = 0
    for p in prompts:
...
def causal_gate_direction(model, layer: int, direction: np.ndarray, payload, *,
                          n: int = 6, alphas=(6.0, 10.0), seed: int = 0) -> CausalResult:
    ...
    # orthogonalize rand against d to get an orthogonal control
    orth = rand - (rand @ d) * d
    ...
    def best_delta(lyr, vec):
        return max(_fire_rate_steered(model, prompts, payload, lyr, vec, a) - base_rate
                   for a in alphas)
    ...
    return CausalResult(ran=True, kind="direction", effect=effect, max_control=max_ctrl,
                        per_control=per, notes=[f"base_rate={base_rate:.2f}",
                                                f"wrong_layer={wrong_layer}"])
